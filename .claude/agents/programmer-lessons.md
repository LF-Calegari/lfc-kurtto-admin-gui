...
- [PR #59] Hex `#ffffff` hardcoded em `Forbidden.module.css` (mesmo erro da #50) — TODA cor em CSS Module deve usar `var(--color-*)` de `variables.css`. Se o token não existe ainda, criar primeiro em `variables.css` alinhado ao `BRAND-GUIDE.md` (que já documenta `--color-white: #FFFFFF` na linha 171), depois consumir. Antes de stagear qualquer `.module.css`, fazer busca por hex literal (`grep -E '#[0-9a-fA-F]{3,6}'`) no diff.
